...
struct IVertexHash{
    std::string operator()(StatePtr k) {
        ...
        return std::to_string((int)round(values[0])) + std::to_string((int)round(values[1])) + std::to_string((int)round(values[2])) + std::to_string((int)round(values[4])) + std::to_string((int)round(values[5]));//debug + std::to_string((int)round(values[6]));
    }
};
