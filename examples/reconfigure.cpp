...
StatePtr fit_state2lattice(StatePtr state, gls::io::MotionPrimitiveReader *mReader,std::shared_ptr<ompl::base::RealVectorStateSpace> space){
    ...
    vals[6] = (double) mReader->ContTheta2DiscNew(vals[6]);
...
std::vector<double> lattice2real(std::vector<double> vals, gls::io::MotionPrimitiveReader *mReader){
    ...
    real_vals[6] = (double) mReader->DiscTheta2ContNew(vals[6]);
...
std::vector<std::tuple<IVertex, VertexProperties, double, int>> transition_function(
    ...
    for (gls::io::MotionPrimitive mprim : mprims){
        ...
        if(mprim.starttheta_c == values[2]){
            ...
            StatePtr newState(new State(space));
            space->copyFromReals(newState->getOMPLState(), std::vector<double>{nx, ny, ntheta, -1, values[4], values[5], values[6]});
            ...
            for(gls::io::pt_xyt point : mprim.intermptV){
                ...
                space->copyFromReals(newState->getOMPLState(), 
                        std::vector<double>{(double)point.x + DISCXY2CONT(values[0], res), 
                                            ...
                                            (double)point.theta, values[3], 
                                            DISCXY2CONT(values[4], res),
                                            DISCXY2CONT(values[5], res),
                                            values[6]});
...
void makeBox(cv::Mat image, 
    ...
    std::pair<double,double> trc = {-dimentions[1]/2.0*std::sin(position[2]), dimentions[1]/2.0*std::cos(position[2])};
    std::pair<double,double> brc = {dimentions[1]/2.0*std::sin(position[2]),-dimentions[1]/2.0*std::cos(position[2])};
...
void displayPath(std::string obstacleFile, std::shared_ptr<ompl::geometric::PathGeometric> path, double resolution, gls::io::MotionPrimitiveReader *mReader) {
  ...
  for (int i = 0; i < pathSize; ++i) {
    ...
    if(i%car_int == 0 || i==pathSize-1 || cutshort){
        ...
        // Car
        makeBox(image, box_color, uvec, {0.44, 0.27}, scale, offsetx, offsety, numberOfRows);
        box_color = cv::Scalar(255, 0, 0);

        // Block
        makeBox(image, cv::Scalar(0,0,0), block_vec, {0.1, 0.1}, scale, offsetx, offsety, numberOfRows);
    }
...
int main (int argc, char const *argv[]) {
  ...
  space->copyFromReals(sourceState->getOMPLState(), std::vector<double>{3, 3, 0, -1, 2, 2, 0});

  StatePtr targetState(new State(space));
  space->copyFromReals(targetState->getOMPLState(), std::vector<double>{2, 3, M_PI, -1, 2, 2, 0});
