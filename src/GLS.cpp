...
// ============================================================================
ConstEventPtr GLS::getEvent() const { return mEvent; }

// ============================================================================
void GLS::setSelector(SelectorPtr selector) { mSelector = selector; }

// ============================================================================
ConstSelectorPtr GLS::getSelector() const { return mSelector; }
...
void GLS::setRoadmap(std::string filename) {
  ...
  mRoadmap = boost::shared_ptr<
      ...
      new io::RoadmapFromFile<Graph, VPStateMap, State, EPLengthMap>(mSpace,
                                                                     filename));
...
CollisionStatus GLS::evaluateVertex(Vertex v) {
  ...
  // Evaluate the state.
  if (!validityChecker->isValid(state)) return CollisionStatus::Collision;
...
void GLS::extendSearchTree() {
  ...
  while (!mExtendQueue.isEmpty()) {
    ...
    if (mGraph[u].getCollisionStatus() == CollisionStatus::Collision) continue;
    ...
    for (boost::tie(ni, ni_end) = adjacent_vertices(u, mGraph); ni != ni_end;
      ...
      if (v == mGraph[u].getParent()) continue;

      // Never come back to the source.
      if (v == mSourceVertex) continue;
      ...
      if (mGraph[v].getVisitStatus() == VisitStatus::NotVisited) {
        ...
        if (oldCostToCome < newCostToCome) continue;

        // Tie-Breaking Rule
        if (oldCostToCome == newCostToCome) {
          if (previousParent < u) continue;
        }
        ...
        if (mExtendQueue.hasVertexWithValue(
          ...
          mExtendQueue.removeVertexWithValue(v,
                                             mGraph[v].getEstimatedTotalCost());
        }
...
void GLS::rewireSearchTree() {
  ...
  for (auto iterS = mRewireSet.begin(); iterS != mRewireSet.end(); ++iterS) {
    ...
    if (mGraph[v].getCollisionStatus() == CollisionStatus::Collision) continue;
    ...
    for (boost::tie(ni, ni_end) = adjacent_vertices(v, mGraph); ni != ni_end;
      ...
      if (mGraph[u].getVisitStatus() == VisitStatus::NotVisited) continue;
  ...
  while (!mRewireQueue.isEmpty()) {
    ...
    for (boost::tie(ni, ni_end) = adjacent_vertices(u, mGraph); ni != ni_end;
      ...
      if (mRewireSet.find(v) == mRewireSet.end()) continue;
...
void GLS::evaluateSearchTree() {
  if (mExtendQueue.isEmpty()) return;
